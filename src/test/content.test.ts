...
import { AP_QUESTION_BANK_SOURCE, apQuestionBank, getAPQuestions } from '../content/apQuestionBank';
...
describe('content integrity',()=>{
 ...
 it('uses only the approved Unit 2 Markdown bank for Practice AP',()=>{expect(AP_QUESTION_BANK_SOURCE).toBe('AP_CHEM_UNIT_2_QUESTION_BANK.md');expect(apQuestionBank).toHaveLength(115);for(const topic of Object.keys(expectedCounts))expect(getAPQuestions(topic as keyof typeof questionBanks)).toHaveLength(15);expect(apQuestionBank[0].prompt).toMatch(/solid sodium chloride/);expect(apQuestionBank.find(q=>q.number===1)?.stimulus).toBeNull();expect(apQuestionBank.find(q=>q.number===107)?.stimulus).toBe('lewis-candidates')});
