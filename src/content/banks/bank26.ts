...
const seeds=cases.flatMap(([species,charges,sum,contributors,observation])=>[
 {prompt:`What formal-charge pattern is associated with a reviewed contributor for ${species}?`,answer:{kind:'text' as const,value:charges,accepted:[charges]},solution:charges,concepts:['formal charge']},
 {prompt:`What must the formal charges sum to for ${species}?`,answer:{kind:'text' as const,value:sum,accepted:[sum,sum.replace('−','-')]},solution:`Formal charges must sum to the overall charge, ${sum}.`,concepts:['charge sum']},
