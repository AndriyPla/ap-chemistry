...
const builderSeeds = structures.map(s => ({
    ...
    answer: { kind: 'lewis', structureId: s.id }, solution: `${s.description} Total valence electrons: ${s.totalValenceElectrons}. ${s.exception ?? ''}`,
    concepts: ['Lewis construction', 'electron counting'], representation: 'lewis',
...
const reasoning = structures.slice(0, 8).flatMap(s => [
    { prompt: `How many total valence electrons must be represented in ${s.formula}?`, answer: { kind: 'number', value: s.totalValenceElectrons, unit: 'electrons', tolerance: 0 }, solution: `The reviewed total is ${s.totalValenceElectrons} valence electrons.`, concepts: ['electron counting'] },
